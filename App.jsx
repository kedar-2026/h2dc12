      ...
      <header className="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex items-center justify-between h-24">
			...
			<button
			  onClick={(e) => { e.stopPropagation(); prevImageInSegment(); }}
			...
			<button
			  onClick={(e) => { e.stopPropagation(); nextImageInSegment(); }}
...
/>
                ...
                <div className="absolute bottom-4 flex gap-1.5 z-10">
                   {productImages[activeProductIndex]?.map((_, idx) => (
                     <div 
                       key={idx} 
                       className={`h-1.5 rounded-full shadow-sm transition-all duration-300 ${idx === activeImageIndex ? 'w-6 bg-blue-600' : 'w-2 bg-slate-300'}`}
                     />
                   ))}
                </div>
                
                <div className={`absolute inset-0 bg-black/10 flex items-center justify-center opacity-0 ${isHovered ? 'opacity-100' : ''} transition-opacity pointer-events-none`}>
                  <span className="bg-black/50 text-white px-3 py-1 rounded-full text-xs backdrop-blur-sm">Paused</span>
                </div>
              </div>
			...
			<ul className="space-y-4 text-slate-600">
			  ...
			  <li className="flex gap-3 items-start">
				...
				<span>
				  ...
				  electrocatalytic hydrogenation of biomass-derived furfural through
